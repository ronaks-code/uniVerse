    ...
    async def main():
        ...
        while True:  # Infinite loop, we'll break it when no more data
            ...
            if not data or data[0]['RETRIEVEDROWS'] == 0:  # If data is empty or RETRIEVEDROWS is 0, we assume there's no more data and break the loop
                break
            filename = date.today().strftime("%b-%d-%Y") + '_' + str(controlNum) + '.json'
            tasks.append(asyncio.to_thread(save_text_to_json_file, data, filename))  # Use asyncio.to_thread to run save_text_to_json_file in a separate thread
            controlNum += 50
